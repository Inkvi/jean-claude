...
describe('sync.ts', () => {
  ...
  describe('syncFromClaudeConfig with file references', () => {
    // Note: These tests use temp directories to avoid touching the real ~/.claude
    // The file reference extraction is tested in settings-parser.test.ts

    it('should handle settings.json without file references', async () => {
      const claudeDir = path.join(tempDir, '.claude');
      const jeanClaudeDir = path.join(tempDir, '.jean-claude');
      ...
      // Create settings.json without any ~/.claude references
      await fs.writeFile(
      ...
      // Settings should be copied as-is
      expect(await fs.pathExists(path.join(jeanClaudeDir, 'settings.json'))).toBe(true);
      const settings = await fs.readFile(path.join(jeanClaudeDir, 'settings.json'), 'utf-8');
      expect(JSON.parse(settings)).toEqual({ theme: 'dark', permissions: { allow: [] } });
    });
    ...
    it('should handle malformed settings.json gracefully', async () => {
      ...
      // Settings should still be copied as-is
      expect(await fs.pathExists(path.join(jeanClaudeDir, 'settings.json'))).toBe(true);
    });
  });
